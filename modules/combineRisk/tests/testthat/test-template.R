...
#    `functionName` is a function's name in your module (e.g., `combineRiskEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/combineRisk/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("combineRisk")
  ...
  parameters <- list(
    ...
    combineRisk = list(.saveInitialTime = NA)
  )
  ...
  if (exists("combineRiskEvent1", envir = .GlobalEnv)) {
    simOutput <- combineRiskEvent1(mySim)
  } else {
  ...
  if (exists("combineRiskEvent2", envir = .GlobalEnv)) {
    simOutput <- combineRiskEvent2(mySim)
  } else {
