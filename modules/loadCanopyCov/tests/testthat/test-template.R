...
#    `functionName` is a function's name in your module (e.g., `loadCanopyCovEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Brian/Projects/Spades/GMRiskMapSpaDES/modules/loadCanopyCov/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loadCanopyCov")
  path <- list(modulePath = "C:/Brian/Projects/Spades/GMRiskMapSpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    loadCanopyCov = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loadCanopyCovEvent1", envir = .GlobalEnv)) {
    simOutput <- loadCanopyCovEvent1(mySim)
  } else {
  ...
  if (exists("loadCanopyCovEvent2", envir = .GlobalEnv)) {
    simOutput <- loadCanopyCovEvent2(mySim)
  } else {
