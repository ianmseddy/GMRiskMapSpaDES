...
#    `functionName` is a function's name in your module (e.g., `loadTreeCoverEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/trapsRepo/modules/loadTreeCover/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loadTreeCover")
  ...
  parameters <- list(
    ...
    loadTreeCover = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loadTreeCoverEvent1", envir = .GlobalEnv)) {
    simOutput <- loadTreeCoverEvent1(mySim)
  } else {
  ...
  if (exists("loadTreeCoverEvent2", envir = .GlobalEnv)) {
    simOutput <- loadTreeCoverEvent2(mySim)
  } else {
