...
#    `functionName` is a function's name in your module (e.g., `lccToTreeCoverEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/trapsRepo/modules/lccToTreeCover/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("lccToTreeCover")
  path <- list(modulePath = "/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/trapsRepo/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    lccToTreeCover = list(.saveInitialTime = NA)
  )
  ...
  if (exists("lccToTreeCoverEvent1", envir = .GlobalEnv)) {
    simOutput <- lccToTreeCoverEvent1(mySim)
  } else {
  ...
  if (exists("lccToTreeCoverEvent2", envir = .GlobalEnv)) {
    simOutput <- lccToTreeCoverEvent2(mySim)
  } else {
