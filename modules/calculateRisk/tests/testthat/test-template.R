...
#    `functionName` is a function's name in your module (e.g., `calculateRiskEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/trapsRepo/modules/calculateRisk/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("calculateRisk")
  ...
  parameters <- list(
    ...
    calculateRisk = list(.saveInitialTime = NA)
  )
  ...
  if (exists("calculateRiskEvent1", envir = .GlobalEnv)) {
    simOutput <- calculateRiskEvent1(mySim)
  } else {
  ...
  if (exists("calculateRiskEvent2", envir = .GlobalEnv)) {
    simOutput <- calculateRiskEvent2(mySim)
  } else {
