...
trapsReportPDFopen <- function(sim) {
  ...
  #IAN: lapply once more through a subset
  for (i in 1:length(sim$ROI)){
    ...
    if("pdf" %in% unlist(strsplit(fileName, "[.]")) == FALSE) {
      fileName[i] <- paste0(fileName[i],".pdf")
    }
      pdf(file.path(outputPath(sim), fileName[i]), width=7, height=9, onefile=TRUE, paper = "letter")
    }
...
trapsReportPDFtrapextent <- function(sim) {
  ...
  for (i in 1:length(sim$ROI)) {
    ...
    # temp_ras <- projectRaster(totalRisk, mapGoogle)
