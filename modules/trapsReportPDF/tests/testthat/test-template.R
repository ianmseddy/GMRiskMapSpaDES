...
#    `functionName` is a function's name in your module (e.g., `trapsReportPDFEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/trapsReportPDF/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("trapsReportPDF")
  ...
  parameters <- list(
    ...
    trapsReportPDF = list(.saveInitialTime = NA)
  )
  ...
  if (exists("trapsReportPDFEvent1", envir = .GlobalEnv)) {
    simOutput <- trapsReportPDFEvent1(mySim)
  } else {
  ...
  if (exists("trapsReportPDFEvent2", envir = .GlobalEnv)) {
    simOutput <- trapsReportPDFEvent2(mySim)
  } else {
