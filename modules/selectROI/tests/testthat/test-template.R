...
#    `functionName` is a function's name in your module (e.g., `selectROIEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/selectROI/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("selectROI")
  ...
  parameters <- list(
    ...
    selectROI = list(.saveInitialTime = NA)
  )
  ...
  if (exists("selectROIEvent1", envir = .GlobalEnv)) {
    simOutput <- selectROIEvent1(mySim)
  } else {
  ...
  if (exists("selectROIEvent2", envir = .GlobalEnv)) {
    simOutput <- selectROIEvent2(mySim)
  } else {
