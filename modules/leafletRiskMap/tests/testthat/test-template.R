...
#    `functionName` is a function's name in your module (e.g., `leafletRiskMapEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/leafletRiskMap/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("leafletRiskMap")
  ...
  parameters <- list(
    ...
    leafletRiskMap = list(.saveInitialTime = NA)
  )
  ...
  if (exists("leafletRiskMapEvent1", envir = .GlobalEnv)) {
    simOutput <- leafletRiskMapEvent1(mySim)
  } else {
  ...
  if (exists("leafletRiskMapEvent2", envir = .GlobalEnv)) {
    simOutput <- leafletRiskMapEvent2(mySim)
  } else {
