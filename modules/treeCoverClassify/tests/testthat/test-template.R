...
#    `functionName` is a function's name in your module (e.g., `treeCoverClassifyEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Van Hezewijk/GMRiskMapSpaDES/modules/treeCoverClassify/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("treeCoverClassify")
  path <- list(modulePath = "C:/Ian/Van Hezewijk/GMRiskMapSpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    treeCoverClassify = list(.saveInitialTime = NA)
  )
  ...
  if (exists("treeCoverClassifyEvent1", envir = .GlobalEnv)) {
    simOutput <- treeCoverClassifyEvent1(mySim)
  } else {
  ...
  if (exists("treeCoverClassifyEvent2", envir = .GlobalEnv)) {
    simOutput <- treeCoverClassifyEvent2(mySim)
  } else {
