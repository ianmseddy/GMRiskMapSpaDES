...
#    `functionName` is a function's name in your module (e.g., `loadPortLocationsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/loadPortLocations/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loadPortLocations")
  ...
  parameters <- list(
    ...
    loadPortLocations = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loadPortLocationsEvent1", envir = .GlobalEnv)) {
    simOutput <- loadPortLocationsEvent1(mySim)
  } else {
  ...
  if (exists("loadPortLocationsEvent2", envir = .GlobalEnv)) {
    simOutput <- loadPortLocationsEvent2(mySim)
  } else {
