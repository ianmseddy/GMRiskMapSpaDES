...
#    `functionName` is a function's name in your module (e.g., `cropReprojectData2Event1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules/cropReprojectData2/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("cropReprojectData2")
  path <- list(modulePath = "/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/GMRiskMapSpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    cropReprojectData2 = list(.saveInitialTime = NA)
  )
  ...
  if (exists("cropReprojectData2Event1", envir = .GlobalEnv)) {
    simOutput <- cropReprojectData2Event1(mySim)
  } else {
  ...
  if (exists("cropReprojectData2Event2", envir = .GlobalEnv)) {
    simOutput <- cropReprojectData2Event2(mySim)
  } else {
