...
#    `functionName` is a function's name in your module (e.g., `loadTrapsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/kaitlynschyrmann/Desktop/SpaDES GM Module 2018/trapsRepo/modules/loadTraps/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loadTraps")
  ...
  parameters <- list(
    ...
    loadTraps = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loadTrapsEvent1", envir = .GlobalEnv)) {
    simOutput <- loadTrapsEvent1(mySim)
  } else {
  ...
  if (exists("loadTrapsEvent2", envir = .GlobalEnv)) {
    simOutput <- loadTrapsEvent2(mySim)
  } else {
